...
const GERMAN_STREETS = [
  'Hauptstraße', 'Bahnhofstraße', 'Schlossstraße', 'Goethestraße', 'Friedrichstraße',
  'Wilhelmstraße', 'Ludwigstraße', 'Königsallee', 'Marienplatz', 'Neue Straße',
  'Marktstraße', 'Kaiserstraße', 'Rathausstraße', 'Kirchstraße', 'Gartenstraße',
  'Mühlenstraße', 'Brückenstraße', 'Bergstraße', 'Talstraße', 'Weinbergstraße',
...
function generateGermanName() {
  const firstNames = ['Max', 'Anna', 'Lukas', 'Mia', 'Felix', 'Emma', 'Noah', 'Lea', 'Paul', 'Sophie'];
...
app.post('/api/send-magic-link', async (req, res) => {
  try {
    ...
    const response = await axios.post(
      ...
      { utc_offset: -420, email_address: email, login_intent: null, locale: 'id-ID', return_to: null, source: 'claude' },
...
app.post('/api/verify-magic-link', async (req, res) => {
  try {
    ...
    const payload = {
      ...
      hcaptcha_token,
      arkose_session_token: '60318c5d5f754c454.6303992704|r=ap-southeast-1|...' // truncated for brevity
    };

    const response = await axios.post(`${CLAUDE_API}/api/auth/verify_magic_link`, payload, {
      headers: { 'accept': '*/*', 'content-type': 'application/json', 'user-agent': 'Mozilla/5.0' }
    });

    const cookies = response.headers['set-cookie'];
    if (cookies) {
      userSessions.set(email, { cookies: cookies.join('; '), userData: response.data });
    }
    res.json(response.data);
  } catch (e) {
...
// ---------- CREATE CHECKOUT SESSION (max_20x) ----------
app.post('/api/create-checkout-session', async (req, res) => {
  try {
    const { email, organization_uuid } = req.body;
    const headers = getHeadersWithSession(email);
    if (!headers.cookie) return res.status(401).json({ error: 'No session' });
...
app.post('/api/submit-payment', async (req, res) => {
  try {
    const { email, sessionId, paymentData } = req.body;
    const headers = getHeadersWithSession(email);
    if (!headers.cookie) return res.status(401).json({ error: 'No session' });
